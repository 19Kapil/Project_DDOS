"""
Network Topology to run an environment to perform various types of DDOS attacks 
"""
    ...
    for i in range(60):
        ...
        for j in range(10):
            ...
            if j <5:
                ...
                info("\ngenerating ICMP traffic between %s and h%s and TCP/UDP traffic between %s and h1" % (src,((dst.split('.'))[3]),src))
            ...
            info("\n%s Downloading index.html from h1" % src)

            src.cmd("wget http://10.0.0.1/index.html")

            info("\n%s Downloading test.zip from h1" % src)
